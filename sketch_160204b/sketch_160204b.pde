size(displayWidth,displayHeight);
noStroke();
//Background decor


fill(#74B3FF);
rect(0,0,10000,600);

fill(#c2b280);
ellipse(640,600,2800,500);

//baseShadow

fill(79);
ellipse(300,550,400,100);

fill(70);
ellipse(300,550,310,60);

fill(50);
ellipse(300,550,260,40);

fill(20);
ellipse(300,550,240,30);



//Portal Shadow

fill(70);
ellipse(1100,550,200,80);

fill(50);
ellipse(1100,550,140,60);

fill(20);
ellipse(1100,550,130,40);
filter( BLUR, 5 );
 
 //Portal Legs
fill(#002CFF);
rect(1120,500,10,50);
fill(#002CFF);
rect(1070,500,10,50);

//portal arms
ellipse(1100,350,180,260);


//Portal
fill(#0365FF);
ellipse(1100,350,120,370);
fill(#8EBAFF);
ellipse(1100,350,100,350);
fill(255);
ellipse(1100,350,80,300);

//portal eyes
fill(#0365FF);
ellipse(1100,250,50,50);
fill(10);
ellipse(1100,250,45,45);
fill(100);
ellipse(1095,240,20,13);
fill(150);
ellipse(1095,240,10,7);
ellipse(1100,320,30,30);
fill(0);
ellipse(1100,320,25,25);



//text
textSize(32);
fill(0);
text("BB-8", 100, 100);
//text
textSize(32);
fill(0);
text("Turret", 900, 100);

//antenna
fill(50);
rect(325,150,2,40);
fill(50);
rect(335,160,3,60);

//head
fill(244);
ellipse(300,250,150,150);
fill(235);
ellipse(300,250,140,140);
fill(230);
ellipse(300,250,130,130);
fill(220);
ellipse(300,250,110,110);
//accents

//body
fill(250);
ellipse(300,400,300,300);
fill(248);
ellipse(300,400,280,280);
fill(245);
ellipse(300,400,270,270);
fill(242);
ellipse(300,400,240,240);
fill(239);
ellipse(300,400,230,230);
//eyering
fill(#FC7303);
ellipse(300,215,50,50);
//eye
fill(0);
ellipse(300,215,40,40);
//eyereflection
fill(120);
ellipse(295,210,20,15);
fill(150);
ellipse(292,209,10,5);
//indicator light ring
fill(120);
ellipse(340,230,20,20);
//indicator light
fill(0);
ellipse(340,230,15,15);
//accents
//L
noStroke();
fill(#FC7303);
ellipse(185,400,70,140);
fill(250);
ellipse(185,400,50,120);
//R
fill(#FC7303);
ellipse(410,400,80,150);
fill(250);
ellipse(410,400,60,130);
//centredot
fill(#FC7303);
ellipse(295,390,140,190);
fill(240);
ellipse(295,390,110,160);


 